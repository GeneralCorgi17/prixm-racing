    ...
    for race in racecards:
        ...
        for r in runners_data:
            runner = {
                ...
                # Capture detailed form history if the API provides it
                'past_results': r.get('past_results', r.get('form_detail', r.get('previous_runs', r.get('results', [])))),
            }
...
def main():
    ...
    if not user or not pwd:
        ...
        os.system(f'python "{os.path.join(OUTPUT_DIR, "scripts", "racecard_fetcher.py")}" --date {target_date}')
        return
